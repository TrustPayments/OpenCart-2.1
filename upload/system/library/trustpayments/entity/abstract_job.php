...
abstract class AbstractJob extends AbstractEntity {
	...
	public function getFailureReason($language = null){
		...
		return \TrustPaymentsHelper::instance($this->registry)->translate($value, $language);
	}
	 ...
	 * @param string $period
	 * @return array|\TrustPayments\Entity\AbstractJob[]
	 */
	...
	public static function markFailedAsDone(\Registry $registry, $order_id){
		...
		if ($db_result->num_rows) {
			...
			\TrustPayments\Entity\Alert::loadFailedJobs($registry)->modifyCount(-($db_result->num_rows));
		}
	}
}