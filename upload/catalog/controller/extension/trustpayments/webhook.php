 ...
 */
class ControllerExtensionTrustPaymentsWebhook extends Controller {

	public function index(){
		$webhook_service = \TrustPayments\Service\Webhook::instance($this->registry);
		...
		try {
			$request = new \TrustPayments\Webhook\Request(json_decode($requestBody));
			$webhook_model = $webhook_service->getWebhookEntityForId($request->getListenerEntityId());
			if ($webhook_model === null) {
				\TrustPaymentsHelper::instance($this->registry)->log(sprintf('Could not retrieve webhook model for listener entity id: %s', $request->getListenerEntityId()), \TrustPaymentsHelper::LOG_ERROR);
			...
			\TrustPaymentsHelper::instance($this->registry)->log("Processing webhook ({$request->getEventId()}) with $webhook_handler_class_name", \TrustPaymentsHelper::LOG_DEBUG);
