...
class LineItem extends AbstractService {
	...
	public function getReducedItemsFromOrder(array $order_info, $transaction_id, $space_id){
		$this->tax = \TrustPaymentsVersionHelper::newTax($this->registry);
		...
		$refund_jobs = \TrustPayments\Entity\RefundJob::loadByOrder($this->registry, $order_info['order_id']);
		$reduction_items = array();
		foreach ($refund_jobs as $refund) {
			if ($refund->getState() != \TrustPayments\Entity\RefundJob::STATE_FAILED_CHECK &&
					$refund->getState() != \TrustPayments\Entity\RefundJob::STATE_FAILED_DONE) {
	...
	public function getItemsFromOrder(array $order_info){
		...
		$order_model = \TrustPaymentsHelper::instance($this->registry)->getOrderModel();
		...
		$shipping_info = \TrustPayments\Entity\ShippingInfo::loadByTransaction($this->registry, $transaction_info->getSpaceId(),
	...
	public function getItemsFromSession(){
		...
		\TrustPaymentsHelper::instance($this->registry)->xfeeproDisableIncVat();
		$this->total = \TrustPaymentsVersionHelper::getSessionTotals($this->registry);
		\TrustPaymentsHelper::instance($this->registry)->xfeeProRestoreIncVat();
		...
		return $this->createLineItems(\TrustPaymentsHelper::instance($this->registry)->getCurrency());
	}
	...
	private function createLineItems($currency_code){
		...
		if ($this->registry->get('currency')->getValue($currency_code) == 1) {
			$expected_total = \TrustPaymentsHelper::instance($this->registry)->formatAmount($expected_total);

			if (!\TrustPaymentsHelper::instance($this->registry)->areAmountsEqual($calculated_total, $expected_total, $currency_code)) {
				if ($this->registry->get('config')->get('trustpayments_rounding_adjustment')) {
					$items[] = $this->createRoundingAdjustmentLineItem($expected_total, $calculated_total);
				}
				else {
					\TrustPaymentsHelper::instance($this->registry)->log(
					...
					\TrustPaymentsHelper::instance($this->registry)->log(array(
	...
	private function createRoundingAdjustmentLineItem($expected, $calculated) {
		...
		$line_item->setName(\TrustPaymentsHelper::instance($this->registry)->getTranslation('rounding_adjustment_item_name'));
		$line_item->setSku('rounding-adjustment');
		$line_item->setUniqueId('rounding-adjustment');
		$line_item->setQuantity(1);
		$line_item->setType(LineItemType::FEE);
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($difference));
	...
	private function createLineItemFromFee($fee, $id){
		...
		$line_item->setType(LineItemType::FEE);
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($fee['amount']));
	...
	private function createLineItemFromTotal($total){
		...
		$line_item->setQuantity(1);
		$line_item->setType(LineItemType::FEE);
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($total['value']));
	...
	private function createXFeeLineItem($total){
		...
		$line_item->setAmountIncludingTax(
				\TrustPaymentsHelper::instance($this->registry)->formatAmount(
						\TrustPaymentsHelper::instance($this->registry)->roundXfeeAmount($total['value'])));
		$taxClass = $this->getXfeeTaxClass($total);
		if ($taxClass) {
			$tax_amount = $this->addTaxesToLineItem($line_item, $total['value'], $taxClass);
			$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($total['value'] + $tax_amount));
		}
		return $this->cleanLineItem($line_item);
	}
	...
	private function createLineItemFromProduct($product){
		...
		if ($this->coupon && (!$this->coupon['product'] || in_array($product['product_id'], $this->coupon['product']))) {
			...
			$line_item->setAttributes(
					array(
						"coupon" => new \TrustPayments\Sdk\Model\LineItemAttributeCreate(
								array(
		...
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($amount_excluding_tax + $tax_amount));
	...
	private function createLineItemFromCoupon(){
		...
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($this->coupon_total));
	...
	private function createLineItemFromVoucher(){
		...
		$line_item->setAmountIncludingTax(\TrustPaymentsHelper::instance($this->registry)->formatAmount($this->voucher['amount']));
