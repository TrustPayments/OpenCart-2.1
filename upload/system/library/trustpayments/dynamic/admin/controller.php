...
abstract class ControllerExtensionPaymentTrustPaymentsBase extends AbstractController {
	...
	public function order(){
		...
		$refund_jobs = \TrustPayments\Entity\RefundJob::loadByOrder($this->registry, $order_id);
		$completion_jobs = \TrustPayments\Entity\CompletionJob::loadByOrder($this->registry, $order_id);
		$void_jobs = \TrustPayments\Entity\VoidJob::loadByOrder($this->registry, $order_id);
		...
		return TrustPaymentsVersionHelper::wrapJobLabels($this->registry, $this->loadView('extension/trustpayments/labels', array(
	...
	private function getTrustPaymentsLink($type, $space, $id, $link_text){
		$base_url = \TrustPaymentsHelper::getBaseUrl();
	...
	private function getDownloadLink($type){
		...
		$url = $this->createUrl('extension/trustpayments/pdf/' . $type,
				array(
					'order_id' => $this->request->get['order_id'],
	...
	private function getTransactionDefaultLabels(\TrustPayments\Entity\TransactionInfo $transaction_info){
		$labels = array(
			'default' => array(
				...
				'labels' => array(
					array(
						'name' => $this->language->get('label_trustpayments_id'),
						'description' => $this->language->get('description_trustpayments_id'),
						'value' => $transaction_info->getTransactionId() 
					),
					array(
						'name' => $this->language->get('label_trustpayments_link'),
						'description' => $this->language->get('description_trustpayments_link'),
						'value' => $this->getTrustPaymentsLink('transaction', $transaction_info->getSpaceId(), $transaction_info->getTransactionId(),
		...
		if ($transaction_info->getState() == \TrustPayments\Sdk\Model\TransactionState::FULFILL) {
			$download_labels[] = array(
	...
	private function getCompletionDefaultLabels(\TrustPayments\Entity\CompletionJob $job){
		$labels = array(
			'default' => array(
				'name' => $this->language->get('label_default'),
	...
	private function addDefaultJobInformation(array &$labels, \TrustPayments\Entity\AbstractJob $job, $link_type){
		$labels['default']['labels'][] = array(
			'name' => $this->language->get('label_status'),
			'description' => $this->language->get('label_status'),
			'value' => $job->getState() 
		...
		if ($job->getJobId()) {
			...
			$labels['default']['labels'][] = array(
				'name' => $this->language->get('label_trustpayments_link'),
				'description' => $this->language->get('description_trustpayments_link'),
				'value' => $this->getTrustPaymentsLink($link_type, $job->getSpaceId(), $job->getJobId(), $this->language->get('link_' . $link_type)) 
			);
		}
	...
	private function getRefundDefaultLabels(\TrustPayments\Entity\RefundJob $job){
		$labels = array(
			'default' => array(
				'name' => $this->language->get('label_default'),
	...
	private function getVoidDefaultLabels(\TrustPayments\Entity\VoidJob $job){
		$labels = array(
			'default' => array(
				'name' => $this->language->get('label_default'),
	...
	private function getLabelTemplateVariables($labels, $display_labels = array()){
		if ($labels) {
			foreach ($labels as $label_id => $label_value) {
				$label_decriptor = \TrustPayments\Provider\LabelDescriptor::instance($this->registry)->find($label_id);
				if(!$label_decriptor) {
					\TrustPaymentsHelper::instance($this->registry)->log("Could not find label descriptor for id $label_id, skipping", \TrustPaymentsHelper::LOG_ERROR);
					continue;
				}
				$group_id = $label_decriptor->getGroup();
				if (!isset($display_labels[$group_id])) {
					$label_group = \TrustPayments\Provider\LabelDescriptionGroup::instance($this->registry)->find($group_id);
					if(!$label_group) {
						\TrustPaymentsHelper::instance($this->registry)->log("Could not find label group for id $group_id, skipping", \TrustPaymentsHelper::LOG_ERROR);
						continue;
					}
					$display_labels[$group_id] = array(
						'name' => htmlspecialchars(\TrustPaymentsHelper::instance($this->registry)->translate($label_group->getName()),
								ENT_HTML5 | ENT_QUOTES),
						'description' => htmlspecialchars(\TrustPaymentsHelper::instance($this->registry)->translate($label_group->getDescription()),
								ENT_HTML5 | ENT_QUOTES),
						'labels' => array()
					);
				}
				$display_labels[$group_id]['labels'][] = array(
					'name' => htmlspecialchars(\TrustPaymentsHelper::instance($this->registry)->translate($label_decriptor->getName()),
							ENT_HTML5 | ENT_QUOTES),
					'description' => htmlspecialchars(\TrustPaymentsHelper::instance($this->registry)->translate($label_decriptor->getDescription()),
							ENT_HTML5 | ENT_QUOTES),
