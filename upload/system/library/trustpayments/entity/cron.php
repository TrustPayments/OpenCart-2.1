...
class Cron extends AbstractEntity {
	...
	public static function cleanUpHangingCrons(\Registry $registry){
		$db = $registry->get('db');
		\TrustPaymentsHelper::instance($registry)->dbTransactionStart();
		$time = new \DateTime();
		...
		try {
			...
			self::query($query, $db);
			\TrustPaymentsHelper::instance($registry)->dbTransactionCommit();
		}
		catch (\Exception $e) {
			\TrustPaymentsHelper::instance($registry)->dbTransactionRollback();
			\TrustPaymentsHelper::instance($registry)->log('Error clean up hanging cron: ' . $e->getMessage());
		}
	}
	...
	public static function insertNewPendingCron(\Registry $registry){
		...
		try {
			...
			$uuid = \TrustPaymentsHelper::generateUuid();
