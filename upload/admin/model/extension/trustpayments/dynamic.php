 ...
 *
 */
class ModelExtensionTrustPaymentsDynamic extends AbstractModel {
		...
		foreach ($stores as $store) {
			...
			foreach ($configurations as $configuration) {
				...
				foreach ($configuration as $partialKey => $value) {
					$settings[TrustPaymentsVersionHelper::extractPaymentSettingCode($configuration['code']) . '_' . $partialKey] = $value;
				}
				$this->model_setting_setting->editSetting(TrustPaymentsVersionHelper::extractPaymentSettingCode($configuration['code']),
	...
	private function loadConfigurations($store_id = null){
		...
		if ($store_id !== null) {
			$space_id = \TrustPaymentsHelper::instance($this->registry)->getSpaceId($store_id);
			$methods = \TrustPayments\Entity\MethodConfiguration::loadBySpaceId($this->registry, $space_id);
		}
		else {
			$methods = \TrustPayments\Entity\MethodConfiguration::loadAll($this->registry);
		}
		foreach ($methods as $method) {
			$configurations[] = array(
				'code' => 'trustpayments_' . $method->getConfigurationId(),
				'title' => $method->getTitle(),
				'sort_order' => $method->getSortOrder(),
				'terms' => '',
				'status' => $method->getState() == \TrustPayments\Entity\MethodConfiguration::STATE_ACTIVE 
			);
		}
	...
	private function createLanguageFiles(array $configuration, $type){
		$source = DIR_SYSTEM . 'library/trustpayments/dynamic/' . $type . '/language.mock';
		...
		foreach ($languages as $code => $language) {
			...
			$restLang = \TrustPayments\Provider\Language::instance($this->registry)->findForStore($language['code'], $language['locale']);
			if (empty($restLang)) {
				\TrustPaymentsHelper::instance($this->registry)->log("Could not find language for code $code.");
				continue;
			}
			...
			$target = str_replace('#code#', TrustPaymentsVersionHelper::extractLanguageDirectory($language), $target_base);
	...
	private function createModel(array $configuration){
		$source = DIR_SYSTEM . 'library/trustpayments/dynamic/catalog/model.mock';
	...
	private function createAdminController(array $configuration){
		$source = DIR_SYSTEM . 'library/trustpayments/dynamic/admin/controller.mock';
	...
	private function createController(array $configuration){
		$source = DIR_SYSTEM . 'library/trustpayments/dynamic/catalog/controller.mock';
